...
for x in range(1,xx,4):
    ...
    if(x % 4 == 1):
        for y in range(1,3):
            ...
            cell = cell_obj.value
            if y == 1:
                sql2 = "UPDATE "+ table +" SET quno = %s WHERE q_id = %s"
                val2 = (cell, res)
                mycursor.execute(sql2, val2)
                mydb.commit()
            elif y == 2:
                sql3 = "UPDATE "+ table +" SET ques = %s WHERE q_id = %s"
                val3 = (cell, res)
    ...
    if((x+2) % 4 == 3):
        for z in range(1,9):
            ...
            cell = cell_obj.value
            if z == 1:
                sql4 = "UPDATE "+ table +" SET ans = %s WHERE q_id = %s"
                val4 = (cell, res)
                mycursor.execute(sql4, val4)
                mydb.commit()
                # print("val4",val4)
            elif z == 2:
                sql5 = "UPDATE "+ table +" SET opt1 = %s WHERE q_id = %s"
                val5 = (cell, res)
                mycursor.execute(sql5, val5)
                mydb.commit()
                # print("val5",val5)
            elif z == 4:
                sql6 = "UPDATE "+ table +" SET opt2 = %s WHERE q_id = %s"
                val6 = (cell, res)
                mycursor.execute(sql6, val6)
                mydb.commit()
                # print("val6",val6)
            elif z == 6:
                sql7 = "UPDATE "+ table +" SET opt3 = %s WHERE q_id = %s"
                val7 = (cell, res)
                mycursor.execute(sql7, val7)
                mydb.commit()
                # print("val7",val7)
            elif z == 8:
                sql8 = "UPDATE "+ table +" SET opt4 = %s WHERE q_id = %s"
                val8 = (cell, res)
